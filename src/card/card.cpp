...
namespace Card
{
  Base::Base(const Player::Base &player, const Player::Stats &stats, const Language &language, const Edition &cardEdition, const Template::Code &templateName)
  ...
  Template::Code Base::getTemplateName() const { return templateName; }

  std::string Base::toString() const
  {
    return "Card(Player: " + player.toString() + ", Stats: " + stats.toString() + ", Language: " + getLanguageCode(language) + ", Edition: " + getCardEditionName(edition) + ", Template: " + Template::toString(templateName) + ")";
  }
